...
public class OptimismTransactionProcessor extends MainnetTransactionProcessor {
  ...
  public TransactionProcessingResult processTransaction(
    ...
    try {
      final var transactionValidator = (OptimismTransactionValidator) transactionValidatorFactory.get();
      ...
      validationResult =
          transactionValidator.validateForSender(opTransaction, sender, transactionValidationParams);
      if (!validationResult.isValid()) {
        LOG.debug("Invalid transaction: {}", validationResult.getErrorMessage());
      ...
      if (opTransaction.getType() != TransactionType.OPTIMISM_DEPOSIT) {
        transactionGasPrice =
            feeMarket.getTransactionPriceCalculator().price(opTransaction, blockHeader.getBaseFee());
      ...
      if (!TransactionType.OPTIMISM_DEPOSIT.equals(opTransaction.getType())) {
        l1CostCalculator
            .l1Cost(blockHeader, opTransaction, worldState);
      ...
      final List<AccessListEntry> accessListEntries = opTransaction.getAccessList().orElse(List.of());
      ...
      final long refundedGas = refunded(opTransaction, initialFrame.getRemainingGas(), baseRefundGas);
      ...
      final long gasUsedByTransaction = opTransaction.getGasLimit() - initialFrame.getRemainingGas();
